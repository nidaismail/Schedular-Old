...
class HomeController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            if (empty($selectedPersonIds) || count($selectedPersonIds) > 1 && is_array($request->input('location'))) {
                $error = 'Please select exactly one person when multiple locations are selected.';
            ...
            foreach ($selectedPersonIds as $selectedPersonId) {
                foreach ($period as $date) {
                    if (in_array($date->format('l'), $request->day)) {
                        foreach ($locationIds as $locationId) {
                            // Create a new schedule for the current person and location
                            $data = new Schedule();
                            $data->date = $date;
